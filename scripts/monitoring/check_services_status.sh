...
then
  telegram_bot --text "🚨 $(hostname): Bitcoin Core is $bitcoind_status"
fi
...
then
  telegram_bot --text "🚨 $(hostname): Electrum Server is $electrum_status"
fi
...
then
  telegram_bot --text "🚨 $(hostname): BTC RPC Explorer is $btcrpcexplorer_status"
fi
...
then
  telegram_bot --text "🚨 $(hostname): Mempool is $mempool_status"
fi
...
then
  telegram_bot --text "🚨 $(hostname): JoinMarket is $jmwalletd_status"
fi
...
then
  telegram_bot --text "🚨 $(hostname): Joinmarket - Orderbook Watcher is $obwatcher_status"
fi
...
then
  telegram_bot --text "🚨 $(hostname): Jam is $jam_status"
fi
...
then
  telegram_bot --text "🚨 $(hostname): Lightning is $lnd_status"
fi
...
then
  telegram_bot --text "🚨 $(hostname): Ride the Lightning is $rtl_status"
fi
