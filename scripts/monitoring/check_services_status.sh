...
then
  telegram_bot --title "🚨 $(hostname):" --text "Bitcoin Core is $bitcoind_status"
fi
...
then
  telegram_bot --title "🚨 $(hostname):" --text "Electrum Server is $electrum_status"
fi
...
then
  telegram_bot --title "🚨 $(hostname):" --text "BTC RPC Explorer is $btcrpcexplorer_status"
fi
...
then
  telegram_bot --title "🚨 $(hostname):" --text "Mempool is $mempool_status"
fi
...
then
  telegram_bot --title "🚨 $(hostname):" --text "JoinMarket is $jmwalletd_status"
fi
...
then
  telegram_bot --title "🚨 $(hostname):" --text "Joinmarket - Orderbook Watcher is $obwatcher_status"
fi
...
then
  telegram_bot --title "🚨 $(hostname):" --text "Jam is $jam_status"
fi
...
then
  telegram_bot --title "🚨 $(hostname):" --text "Lightning is $lnd_status"
fi
...
then
  telegram_bot --title "🚨 $(hostname):" --text "Ride the Lightning is $rtl_status"
fi
